...
public class BioticStatShockwave extends AbstractBioticStat {
   ...
   public static final int DELAY = 20 * 5;
    ...
    public String getDetails(int level) {
        return String.format(super.getDetails(level), EnumChatFormatting.YELLOW + Integer.toString(12) + EnumChatFormatting.GRAY);
    }
    ...
    public void onActionKeyPress(AndroidPlayer androidPlayer, int level, boolean server) {
        if (androidPlayer.getActiveStat() == this && server) {
            createShockwave(androidPlayer, androidPlayer.getPlayer(), 20);
        }
    }
        ...
        if (getLastShockwaveTime(androidPlayer) < androidPlayer.getPlayer().worldObj.getTotalWorldTime()) {
            if (!MinecraftForge.EVENT_BUS.post(new MOEventBionicStat(this, androidPlayer.getUnlockedLevel(this), androidPlayer))) {
                if (!entityPlayer.worldObj.isRemote) {
                    float range = MathHelper.clamp_float(distance, 12, 12);
                    float power = MathHelper.clamp_float(distance, 3, 3) * 1.0f;
                    ...
                    for (EntityLivingBase entityLivingBase : entities) {
                        if (entityLivingBase != entityPlayer) {
                            ...
                            dir = dir.normalize();
                            entityLivingBase.addVelocity(dir.xCoord * power * 0.15f * distanceMultiply, power * 0.15f, dir.zCoord * power * 0.15f * distanceMultiply);
                            entityLivingBase.velocityChanged = true;
                            ShockwaveDamage damageSource = new ShockwaveDamage("android_shockwave", entityPlayer);
                            entityLivingBase.attackEntityFrom(damageSource, power * 5);
                        }
                    }
