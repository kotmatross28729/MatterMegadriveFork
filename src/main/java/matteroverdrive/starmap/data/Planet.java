...
public class Planet extends SpaceBody implements IInventory {
    ...
    public void setOwner(EntityPlayer player) {
        ownerUUID = EntityPlayer.func_146094_a(player.getGameProfile());
    }
    ...
    public boolean isOwner(EntityPlayer player) {
        if (hasOwner()) return getOwnerUUID().equals(EntityPlayer.func_146094_a(player.getGameProfile()));
        return false;
    }
    ...
    public boolean isHomeworld(EntityPlayer player) {
        if (isOwner(player)) return isHomeworld();
        else return false;
    }
    ...
    public ItemStack removeShip(int at) {
        if (at < fleet.size()) return fleet.remove(at);
        else return null;
    }
